...
(:predicates (located_at ?x - rover ?y - waypoint) 
...
:duration (= ?duration 5)
:condition (and (over all (can_traverse ?x ?y ?z)) (at start (available ?x)) (at start (located_at ?x ?y))  (at start (>= (energy ?x) 8))
                (over all (visible ?y ?z))
	    )
:effect (and (at start (decrease (energy ?x) 8)) (at start (not (located_at ?x ?y))) (at end (located_at ?x ?z))))

(:durative-action recharge
:parameters (?x - rover ?w - waypoint)
:duration (= ?duration (/ ( - 80 (energy ?x)) (recharge-rate ?x)))
:condition (and (at start (located_at ?x ?w)) (over all (located_at ?x ?w)) 
...
:condition (and (over all (located_at ?x ?p)) (at start (located_at ?x ?p)) (at start (at_soil_sample ?p)) (at start (equipped_for_soil_analysis ?x)) (at start (>= (energy ?x) 3)) (at start (store_of ?s ?x)) (at start (empty ?s))
		)
...
:parameters (?x - rover ?s - store ?p - waypoint)
:duration (= ?duration 8)
:condition (and (over all (located_at ?x ?p)) (at start (located_at ?x ?p)) (at start (>= (energy ?x) 5)) (at start (at_rock_sample ?p)) (at start (equipped_for_rock_analysis ?x)) (at start (store_of ?s ?x)) (at start (empty ?s))
		)
...
(:durative-action calibrate
 ...
 :duration (= ?duration 5)
 :condition (and (at start (equipped_for_imaging ?r)) (at start (>= (energy ?r) 2)) (at start (calibration_target ?i ?t)) (over all (located_at ?r ?w)) (at start (visible_from ?t ?w)) (at start (on_board ?i ?r))
		)
...
(:durative-action communicate_soil_data
 ...
 :duration (= ?duration 10)
 :condition (and (over all (located_at ?r ?x)) (over all (at_lander ?l ?y)) (at start (have_soil_analysis ?r ?p)) (at start (>= (energy ?r) 4))
...
(:durative-action communicate_rock_data
 :parameters (?r - rover ?l - lander ?p - waypoint ?x - waypoint ?y - waypoint)
 :duration (= ?duration 10)
 :condition (and (over all (located_at ?r ?x)) (over all (at_lander ?l ?y)) (at start (have_rock_analysis ?r ?p)) 
...
(:durative-action communicate_image_data
 ...
 :condition (and (over all (located_at ?r ?x)) (over all (at_lander ?l ?y)) (at start (have_image ?r ?o ?m)) 
